...
public class KernelService extends AbstractDataSource {
    ...
    public static interface FailedRequestStrategy {
        ...
        public static class ExponentialBackoff implements FailedRequestStrategy {
            ...
            @Override
            public <T extends DataObject> T onFailedRequest(final KernelService service, final RequestContext<T> context, final Response response,
        ...
        public <T extends DataObject> T onFailedRequest(KernelService service, RequestContext<T> context, Response response, OriannaException e);
    }
    ...
    protected <T extends DataObject> T get(final Class<T> type, final String endpoint) {
    ...
    protected <T extends DataObject> T get(final Class<T> type, final String endpoint, final Multimap<String, String> parameters) {
    ...
    private <T extends DataObject> T get(final RequestContext<T> context) {
        ...
        return msgpack ? DataObject.fromBytes(context.type, response.getBytes()) : DataObject.fromJSON(context.type, response.getBody());
    }
}
